...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['id_sobremesa'])) {
    ...
    if ($quantidade <= $quantidade_disponivel) {
        ...
        $stmt_atualiza = $mysqli->prepare("UPDATE sobremesa SET quantidade = ? WHERE id_sobremesa = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['esvaziar_carrinho'])) {
    foreach ($_SESSION['carrinho'] as $item) {
        ...
        $stmt_reverte = $mysqli->prepare("SELECT quantidade FROM sobremesa WHERE id_sobremesa = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['finalizar_pedido'])) {
    ...
    foreach ($_SESSION['carrinho'] as $item) {
        ...
        if ($stmt_item) {
            $stmt_item->bind_param("iii", $id_pedido, $item['id_sobremesa'], $quantidade);
            $stmt_item->execute();
            $stmt_item->close();
        } else {
