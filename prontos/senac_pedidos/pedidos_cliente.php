...
if (isset($_SESSION["id_cliente"])) {
    $stmt = $mysqli->prepare("SELECT id_pedido, data_pedido, status FROM pi_2024_pedidos_pedidos WHERE id_clientes = ?");
...
if (isset($_POST['cancelar_pedido'])) {
    ...
    $stmt = $mysqli->prepare("UPDATE pi_2024_pedidos_pedidos SET status = 'Cancelado' WHERE id_pedido = ? AND id_clientes = ?");
                        ...
                        <table class="table table-striped mt-4">
                            ...
                            <tbody>
                                <?php while ($pedido = $result->fetch_assoc()): ?>
                                    ...
                                    $stmt_itens = $mysqli->prepare("SELECT id_lanches, id, id_sobremesa, quantidade FROM pi_2024_pedidos_itens_pedido WHERE id_pedido = ?");
                                    ...
                                    while ($item = $result_itens->fetch_assoc()) {
                                        ...
                                        if (!empty($item['id_bebidas'])) {
                                            $stmt_produto = $mysqli->prepare("SELECT nome, preco FROM pi_2024_pedidos_bebidas WHERE id_bebidas = ?");
