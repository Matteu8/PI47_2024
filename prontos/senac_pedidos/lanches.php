...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['id_lanche'])) {
    ...
    $stmt_verifica = $mysqli->prepare("SELECT quantidade FROM pi_2024_pedidos_lanches WHERE id_lanches = ?");
    ...
    if ($quantidade <= $quantidade_disponivel) {
        ...
        $stmt_atualiza = $mysqli->prepare("UPDATE pi_2024_pedidos_lanches SET quantidade = ? WHERE id_lanches = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['esvaziar_carrinho'])) {
    foreach ($_SESSION['carrinho'] as $item) {
        ...
        $stmt_reverte = $mysqli->prepare("SELECT quantidade FROM pi_2024_pedidos_lanches WHERE id_lanches = ?");
...
$resultado = $mysqli->query("SELECT * FROM pi_2024_pedidos_lanches");
//var_dump($item);
?>
