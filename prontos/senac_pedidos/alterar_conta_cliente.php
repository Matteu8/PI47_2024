...
if (isset($_SESSION["id_cliente"])) {
    ...
    if (isset($_POST['id_clientes'])) {
        ...
        $email_check = $mysqli->prepare("SELECT id_clientes FROM pi_2024_pedidos_clientes WHERE email = ? AND id_clientes != ?");
        ...
        if ($email_result->num_rows > 0) {
            ...
            if (!empty($senha)) {
                ...
                $stmt = $mysqli->prepare("UPDATE pi_2024_pedidos_clientes SET nome = ?, telefone = ?, curso = ?, periodo = ?, email = ?, senha = ? WHERE id_clientes = ?");
                $stmt->bind_param("ssssssi", $nome, $telefone, $curso, $periodo, $email, $senha, $id_clientes);
            } else {
                $stmt = $mysqli->prepare("UPDATE pi_2024_pedidos_clientes SET nome = ?, telefone = ?, curso = ?, periodo = ?, email = ? WHERE id_clientes = ?");
