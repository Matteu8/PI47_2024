...
if (isset($_SESSION["id_funcionario"])) {
    ...
    $stmt_total = $mysqli->prepare("SELECT COUNT(*) AS total FROM pi_2024_pedidos_pedidos");
    $stmt_total->execute();
    ...
    $stmt = $mysqli->prepare("SELECT id_pedido, data_pedido, status, id_clientes, nome_funcionario FROM pi_2024_pedidos_pedidos LIMIT ?, ?");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['concluir_pedido'])) {
        ...
        $stmt_concluir = $mysqli->prepare("UPDATE pi_2024_pedidos_pedidos SET status = 'Pedido Concluído' WHERE id_pedido = ?");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['remover_pedido'])) {
        $id_pedido = $_POST['id_pedido'];
        $stmt_remover = $mysqli->prepare("DELETE FROM pi_2024_pedidos_pedidos WHERE id_pedido = ?");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['esvaziar_tabela'])) {
        $stmt_truncate_items = $mysqli->prepare("TRUNCATE TABLE pi_2024_pedidos_itens_pedido");
        $stmt_truncate_items->execute();
        $stmt_truncate_items->close();
        
        $stmt_truncate_pedidos = $mysqli->prepare("TRUNCATE TABLE pi_2024_pedidos_pedidos");
...
<body>
    ...
    <div class="container">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php while ($pedido = $result->fetch_assoc()): ?>
                            ...
                            $stmt_itens = $mysqli->prepare("SELECT id_item_pedido, id_lanches, id_sobremesa, id, quantidade FROM pi_2024_pedidos_itens_pedido WHERE id_pedido = ?");
                            $stmt_itens->bind_param("i", $pedido['id_pedido']);
                            $stmt_itens->execute();
                            $result_itens = $stmt_itens->get_result();

                            // Obter nome do cliente
                            $stmt_cliente = $mysqli->prepare("SELECT nome FROM pi_2024_pedidos_clientes WHERE id_clientes = ?");
                            ...
                            while ($item = $result_itens->fetch_assoc()) {
                                ...
                                if (!empty($item['id_lanches'])) {
                                    ...
                                    $nome_produto = $produto ? htmlspecialchars($produto['nome']) : 'Não tem nada 3';
                                    $preco = $produto ? $produto['preco'] : 0;
                                }elseif(!empty($item['id_sobremesa'])){
                                    $stmt_produto = $mysqli->prepare("SELECT nome, preco FROM pi_2024_pedidos_sobremesa WHERE id_sobremesa = ?");
                                    $stmt_produto->bind_param("i", $item['id_sobremesa']);
                                    $stmt_produto->execute();
                                    $resultado_produto = $stmt_produto->get_result();
                                    $produto = $resultado_produto->fetch_assoc();
                                    
                                    $nome_produto = $produto ? htmlspecialchars($produto['nome']) : 'Não tem nada';
                                    $preco = $produto ? $produto['preco'] : 0;
                                }elseif(!empty($item['id'])){
                                    $stmt_produto = $mysqli->prepare("SELECT nome, preco FROM pi_2024_pedidos_bebidas WHERE id = ?");
                                    $stmt_produto->bind_param("i", $item['id']);
                                    $stmt_produto->execute();
                                    $resultado_produto = $stmt_produto->get_result();
                                    $produto = $resultado_produto->fetch_assoc();
                                   
                                    $nome_produto = $produto ? htmlspecialchars($produto['nome']) : 'Não tem nada 2';
                                    $preco = $produto ? $produto['preco'] : 0;
        ...
        <div class="mt-4 text-center">
            <form method="post">
                <input type="submit" name="esvaziar_tabela" class="btn btn-warning" value="Esvaziar Tabela">
            </form>
        </div>
        <div class="mt-4 text-center">
