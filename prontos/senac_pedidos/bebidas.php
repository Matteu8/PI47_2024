...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['id'])) {
    ...
    $stmt_verifica = $mysqli->prepare("SELECT quantidade FROM pi_2024_pedidos_bebidas WHERE id = ?");
    ...
    if ($quantidade <= $quantidade_disponivel) {
        ...
        $stmt_atualiza = $mysqli->prepare("UPDATE pi_2024_pedidos_bebidas SET quantidade = ? WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['esvaziar_carrinho'])) {
    foreach ($_SESSION['carrinho'] as $item) {
        ...
        $stmt_reverte = $mysqli->prepare("SELECT quantidade FROM pi_2024_pedidos_bebidas WHERE id = ?");
...
$resultado = $mysqli->query("SELECT * FROM pi_2024_pedidos_bebidas");
?>
